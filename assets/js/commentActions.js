function postComment(button, postedBy, videoId, replyTo, containerClass,sessionVariables) {
    ...
    if(commentText) {
        ...
        $.post("ajax/postComment.php", { commentText: commentText, postedBy: postedBy,
            videoId: videoId, responseTo: replyTo, userData: JSON.stringify(sessionVariables) })
            .done(function(comment){

                if(containerClass==='comments'){
                    $("." + containerClass).prepend(comment);
                }
                else{
                    //If containerClass = repliesSection
                    var parent = $(button).closest(".itemContainer");
                    var repliesSection = parent.find("."+containerClass).first();
                    repliesSection.prepend(comment);
                }
                // alert(comment);
            });
